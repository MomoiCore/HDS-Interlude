...
function toPromptPayload(request) {
  return {
    ...
    recentLogicalTurns: request.recentLogicalTurns.map((turn) => ({
      ...
      characterMessages: turn.characterMessages,
    ...
    currentEvent: request.phase === "advance" || request.phase === "conversation-follow-up" ? { type: "none" } : request.groupContext ? { type: "group-message-batch", content: request.userMessage ?? "" } : request.phase === "user-message" ? { type: "private-message-batch", content: request.userMessage ?? "", imageCount: request.images?.length ?? 0 } : { type: "due-intents" },
...
function collectParticipantKnownFacts(phase, participant, entries, facts, from, now, limit, characterBudget) {
  const selected = [];
...
function normalizeSceneTrace(value) {
  ...
  if (!situation && !actions.length && !details.length && !unfinished.length && !participantFacts.length) return void 0;
...
var Runtime = import_koishi2.Schema.object({
  ...
  contextEntryLimit: import_koishi2.Schema.natural().min(1).max(200).default(30).description("主模型最多读取的近期事实卡数量。不会把原剧本正文作为实时上下文。"),
  contextCharacterBudget: import_koishi2.Schema.natural().min(1e3).max(5e4).default(6e3).description("近期事实卡与短期对话上下文的合计字符预算。默认 6000 通常足以保留近期小事。"),
  interactionLedgerLimit: import_koishi2.Schema.natural().min(2).max(24).default(12).description("近期逻辑回合卡的最大数量。一张卡合并一批用户消息、主角已送达的分段消息和当时生活状态。"),
  interactionLedgerCharacterBudget: import_koishi2.Schema.natural().min(200).max(4e3).default(2400).description("近期逻辑回合卡的字符预算。卡片只保留事实和已送达消息，不回传原剧本正文。建议 1800–2400。"),
