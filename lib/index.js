...
function selectActiveScenePromptEntries(entries, characterBudget, narrativeLimit) {
  ...
  return uniqueSceneEntries([...selectedScripts, ...pinned]).sort(compareSceneEntry);
}
__name(selectActiveScenePromptEntries, "selectActiveScenePromptEntries");
function nearDuplicateProse(left, right) {
...
__name(nearDuplicateProse, "nearDuplicateProse");
...
function toPromptPayload(request) {
  ...
  const recentDialogueCandidates = request.participant ? request.recentLogicalTurns.filter((turn) => turn.participantId === request.participant.id && turn.userMessages.length).slice(-10).map((turn) => ({
    entryId: turn.entryId,
    time: storyTimeDescriptor(turn.occurredAt, request.now, request.story.setting.timezone),
...
${message.content}`).join("\n\n");
  ...
  async decide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], extraWebContext = [], abortSignal) {
    ...
    if (repeated || repeatedNarrative) {
      ...
      this.reportOperation(
        ...
        correctionReason,
        repeated?.previous.entryId ?? repeatedNarrative?.previous.id ?? "?"
      );
  ...
  async recordCharacterMessage(participant, now, decisionAt = now) {
    const current = normalizeParticipantState(participant.state);
...
function collectRecentLifeFacts(entries, now, hours, limit, characterBudget, timezone, excludeEntryIds) {
