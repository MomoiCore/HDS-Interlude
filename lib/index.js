...
function systemPrompt(phase, mainPrompt, formatPrompt, fixedPrompt, baseStylePrompt, storyStylePrompt, hookUpdate = "none") {
  return [
...
function toPromptPayload(request) {
  ...
  const currentEvent = request.phase === "advance" || request.phase === "conversation-follow-up" ? { type: "none" } : request.groupContext ? { type: "group-message-batch", content: request.userMessage ?? "" } : request.phase === "user-message" ? {
  ...
  return {
    ...
    recentLogicalTurns: recentLogicalTurns.length ? recentLogicalTurns : void 0,
...
var InterludeService = class extends import_koishi.Service {
   ...
   * the model.
   */
  async recentSceneTraceEntries(storyId, limit = 80) {
  ...
  async recentLogicalTurns(storyId, participantId = "") {
    ...
    return collectRecentLogicalTurns(
      ...
      participantId,
      limit,
      Math.max(1800, Math.min(4e3, this.config.runtime.interactionLedgerCharacterBudget ?? 2400))
    );
  }
  ...
  async recentLifeFacts(storyId, participantId = "") {
    ...
    return collectRecentLifeFacts(
      ...
      Math.max(600, Math.min(6e3, this.config.runtime.recentLifeFactCharacterBudget ?? 2400)),
      /* @__PURE__ */ new Set()
    );
  }
...
${message.content}`).join("\n\n");
  ...
  async advanceUnlocked(story, now, force) {
    ...
    if (elapsed > 0 && !hasNarrativeDue && (force || automaticDue && !pausedForConversation)) {
      ...
      const { decision, hookUpdate, succeeded } = await this.tryDecide(story, followUpParticipant ?? null, phase, from, now, void 0, []);
      if (succeeded) {
    ...
    if (dueBatch) {
      ...
      const { decision, hookUpdate, succeeded } = await this.tryDecide(current, dueParticipant ?? null, "intent-due", dueFrom, now, void 0, dueBatch);
  ...
  async decide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], extraWebContext = []) {
    ...
    const recentLifeFactCharacterBudget = Math.max(600, Math.min(6e3, this.config.runtime.recentLifeFactCharacterBudget ?? 2400));
    ...
    const [recentEntries, memories, scene, arc, facts, allParticipants, webContext, activeConsequences, overlaySnapshots, recentSceneEntries] = await Promise.all([
      ...
      storyState.storyHook ? Promise.resolve(void 0) : this.activeScene(story.id),
      ...
      recentLifeFactsEnabled ? this.recentSceneTraceEntries(story.id, 80) : Promise.resolve([])
    ]);
  ...
  async tryDecide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = []) {
    ...
    try {
      ...
      return { decision: {}, hookUpdate: "none", succeeded: false, effectiveNow, immediateObservations };
    }
  }
  async persistDecision(story, participant, raw, from, now, permitMessages, phase, observedContext = "", hookUpdate = "none") {
...
function collectRecentLifeFacts(entries, now, hours, limit, characterBudget, excludeEntryIds) {
...
function recentLifeFactSize(fact) {
  return fact.situation.length + fact.actions.reduce((sum, item) => sum + item.length, 0) + fact.details.reduce((sum, item) => sum + item.length, 0) + fact.unfinished.reduce((sum, item) => sum + item.length, 0) + (fact.exchange?.userMeaning?.length ?? 0);
}
...
function logicalTurnCardSize(card) {
  return card.situation.length + card.actions.reduce((sum, item) => sum + item.length, 0) + card.details.reduce((sum, item) => sum + item.length, 0) + card.unfinished.reduce((sum, item) => sum + item.length, 0) + card.userMessages.reduce((sum, item) => sum + item.length, 0) + (card.exchange?.userMeaning?.length ?? 0) + (card.exchange?.responseMeaning?.length ?? 0) + (card.participantFacts ?? []).reduce((sum, item) => sum + item.fact.length, 0);
}
...
function normalizeSceneTrace(value) {
  ...
  if (!situation && !actions.length && !details.length && !unfinished.length && !exchange && !participantFacts.length) return void 0;
...
function normalizeConversationTrace(value) {
  ...
  if (!userMeaning && !responseMeaning && !status) return void 0;
  return { userMeaning: userMeaning || void 0, responseMeaning: responseMeaning || void 0, status };
}
...
function normalizeStoryState(value) {
  ...
  return {
    ...
    storyHook,
    storyHookEntryId: typeof record.storyHookEntryId === "number" && Number.isFinite(record.storyHookEntryId) ? Math.max(0, Math.floor(record.storyHookEntryId)) : void 0,
    ...
    lastStoryHookUpdateAt: typeof record.lastStoryHookUpdateAt === "string" ? record.lastStoryHookUpdateAt : typeof record.lastContinuityUpdateAt === "string" ? record.lastContinuityUpdateAt : void 0,
    lastStoryHookPatchAt: typeof record.lastStoryHookPatchAt === "string" ? record.lastStoryHookPatchAt : void 0,
    automation: {
...
var Runtime = import_koishi2.Schema.object({
  ...
  contextEntryLimit: import_koishi2.Schema.natural().min(1).max(200).default(30).description("主模型扫描近期记录的数量上限。不会把原剧本正文作为实时上下文。"),
  ...
  interactionLedgerLimit: import_koishi2.Schema.natural().min(2).max(24).default(12).description("近期逻辑回合卡数量。它保留刚发生的用户语义、已送达结论与当时生活状态。"),
  interactionLedgerCharacterBudget: import_koishi2.Schema.natural().min(200).max(4e3).default(2400).description("近期逻辑回合卡字符预算。它不回传旧剧本文字或历史回复原句。建议 1800–2400。"),
  recentLifeFactsEnabled: import_koishi2.Schema.boolean().default(true).description("保留最近生活事实桥。它让主模型知道昨晚或当天较早发生的具体事情，不会回传旧剧本文字。"),
  recentLifeFactHours: import_koishi2.Schema.natural().min(6).max(168).default(36).description("近期生活事实保留的时间范围，默认 36 小时，足以覆盖昨晚到今天的连续生活。"),
  recentLifeFactLimit: import_koishi2.Schema.natural().min(4).max(60).default(20).description("单次主模型最多读取多少条较早的生活事实。数值更高会增强日内连续性，也会增加上下文。"),
  recentLifeFactCharacterBudget: import_koishi2.Schema.natural().min(600).max(6e3).default(2400).description("近期生活事实的字符预算。默认 2400；优先保留动作、地点、人物、约定与未完事项。"),
...
function registerCommands(ctx, service) {
  ...
  ctx.command("interlude.context", "查看剧本引子、近期逻辑回合卡、场景摘要、人物变化和长期事实").action(async ({ session }) => {
    const story = await requireStory(service, session);
    if (typeof story === "string") return story;
    const participant = await service.findParticipant(session, story);
