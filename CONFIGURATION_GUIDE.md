...
### 2.4.1 活动场景原始时间线与兼容回退

正常写作以 `activeScene` 为唯一短期连续性源：它包含最近剧本原文、其间实际收到的用户消息和已成功投递的角色消息，且按发生时间排列。`currentEvent` 只承载本轮新消息，并会从活动场景中排除，避免同一事件出现两次。`recentSceneProgress` 和 `recentDialogue` 保留给旧数据或没有可用活动场景的兼容回退，不会与完整活动场景叠加。

`runtime.activeSceneEntryLimit` 是数据库扫描上限；`runtime.activeSceneNarrativeLimit` 控制实际提供给主模型的最近剧本段落数，默认 `20`；`runtime.activeSceneCharacterBudget` 控制字符预算。`runtime.interactionLedgerLimit` 与 `runtime.interactionLedgerCharacterBudget` 控制本地逻辑回合缓存，用于诊断、重复校正和兼容回退。只有适配器确认投递成功的消息才能进入活动场景；草稿和失败投递不会被当作已经说出口的话。

上下文组装不会合并或改写活动场景中的既有剧本文字和真实消息，以保留完整的短期因果顺序。活动场景不足 20 段时，会从上一场景尾部补足可用的转场内容。
...
`runtime.recentLifeFactsEnabled` 会额外提供活动场景之外、但仍处于近期时间窗内的生活事实。它从 `sceneTrace` 中读取时间、地点、行动、配角互动、承诺与未完事项；整个活动场景都会被排除，未投递草稿也不会进入。
...
| `interactionLedgerLimit` | 近期逻辑回合缓存数量，默认 `18`；用于重复校正、诊断和缺少活动场景时的兼容回退。 |
| `interactionLedgerCharacterBudget` | 近期逻辑回合缓存字符预算，默认 `3600`；保存动作、锚点、外部影响与交流含义。 |
...
活动场景是实时写作的短期连续性源；逻辑回合是本地状态缓存。每次主叙事都会返回 `sceneState`，记录当前地点、主要活动、具体动作、刚完成或暂停的动作、身体、情绪、注意力、涉及人物和未完事项。模型遗漏细分字段时，插件会从本轮已完成动作和上一场景状态补全连续性前沿。短时间密集对话只追加实际发生的增量；睡眠与醒来、明显换地点、主要活动完成或生活阶段转换等真实边界才会请求 `close-and-open`。
...
`storyHook` 只承担活动场景之前的稳定历史。最后一次约第 10/20 分钟的短期推进会返回小型 `storyHookPatch`；连续空闲达到 `storyHookFullRefreshIdleMinutes` 后，下一次常规推进才完整重写。活动场景、场景外事实桥和 Hook 按时间层级分工，避免同一句角色发言在多个字段中重复出现。
