...
`runtime.interactionLedgerLimit` 与 `runtime.interactionLedgerCharacterBudget` 控制主模型看到的近期逻辑回合卡。每张卡合并一次写作回合中的主角当前处境、已经完成的行动、具体细节、未完成事项、该批用户消息，以及“用户表达的含义 / 主角已经传达的结论”。`<sep/>` 的多个气泡仍归入同一张卡，但其原句仅保留在数据库审计记录中，避免成为下一轮的重复措辞示例。
...
`runtime.recentLifeFactsEnabled` 会额外向主模型提供最近一段真实时间内、早于即时对话卡的生活事实。它从每次已完成写作的 `sceneTrace` 中读取时间、地点、已完成行动、配角互动、具体细节、承诺与未完事项，不读取旧剧本正文、历史角色原句或未投递草稿。

默认保留最近 36 小时、最多 20 条、2400 个字符。这一层专门用于处理“昨晚”“今天早些时候”“刚才和谁在做什么”之类的引用：主模型可以沿着事实时间线回忆，而不用临时补出一个场景。它只增加一次轻量的脚本事实读取，不增加模型调用；希望覆盖更长的安静期时可将小时数提高到 48～72，并相应控制字符预算。
...
| `interactionLedgerCharacterBudget` | 近期逻辑回合卡字符预算，默认 `2400`。主角近期行动、生活状态、用户实际消息和交流结论会占用此预算；旧回复原句不占实时提示词预算。 |
| `recentLifeFactsEnabled` | 是否启用近期生活事实桥。默认开启；关闭后模型只依赖剧本引子、即时逻辑回合和长期记忆。 |
| `recentLifeFactHours` | 近期生活事实覆盖时长，默认 `36` 小时。用于保留昨晚到今天的连续生活。 |
| `recentLifeFactLimit` | 每次最多附带多少条较早生活事实，默认 `20`。提高会增强日内连贯性并增加上下文。 |
| `recentLifeFactCharacterBudget` | 近期生活事实字符预算，默认 `2400`。优先保留行动、地点、人物、约定和未完事项。 |
...
每次主叙事在输出正文的同时生成 `sceneTrace`，以事实短句记录本回合结束时的处境、已完成的主角行动、具体细节、未完成事项与交流语义结果。插件会把它与该回合真实收到的用户消息、适配器确认送达状态合并为一张近期逻辑回合卡；同时将较早但仍在最近时间窗内的卡提炼为“近期生活事实桥”。卡片保存在对应剧本条目的 metadata 中，不增加模型调用，也不替代原始剧本。已送达消息正文仍可用于审计和管理，但不会反复发送给主模型模仿。
