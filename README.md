...
| 活动场景写作源 | 当前场景最近约 20 段剧本原文，以及发生在其间的真实用户消息和已成功投递的角色消息 | 单一、按时间排序的短期事实源；保留细节、动作和对话衔接 |
| 上一场景尾部 | 最近结束场景的末尾原文 | 为睡醒、换地点、活动结束等转场保留衔接 |
| 近期逻辑回合 | 最近若干次写作的状态、关注面、已完成动作、具体锚点和外部事件影响 | 用于本地诊断、重复校正和无活动场景时的兼容回退；不与完整活动场景重复输入 |
...
5. 组装单次主模型请求：时间段、当前参与者关系、按时间排序的活动场景记录、当前真实事件、场景外近期事实、长期事实、其他账号状态、演化覆写和到期意图。活动场景把剧本与已确认收发消息放在同一条时间线上；当前新消息只通过 `currentEvent` 传入一次。
...
- interaction 先说明角色此刻为何有或没有回复条件，再给出是否看见、是否回复、回复方式和延迟时间。
...
~~~json
{
  ...
  "interaction": {
    "seen": true,
    "reason": "她正在收尾，但昨晚的承诺使这条消息值得在忙完后处理。",
    "reply": {
      "mode": "delayed",
      "content": "我刚有点事。",
...
`script` 保存剧本正文，`sceneState` 决定当前场景是否延续，`eventResults` 记录真实事件是否仍待处理，`interaction` 先给出本轮注意力/回复理由，再保存机器可执行的互动结果。`reason` 只用于同一次写作的联系决策，不是额外模型调用，也不会被当作长期人设。只有真实用户事件 id 能进入事件状态；只有适配器确认成功的角色消息会成为下一轮既成事实。
...
主模型读取的是“活动场景原始时间线 + 分层历史”的连续性包：

1. `activeScene`：当前场景最近若干段剧本原文、其间实际收到的用户消息，以及适配器确认投递成功的角色消息，严格按发生时间排序。
2. `previousSceneTail`：上一场景末尾，用于延续转场后的动作、情绪和未完成因果。
3. `currentEvent.messages`：本轮新收到的唯一实时事件。相同条目会从 `activeScene` 中暂时排除，避免重复。
4. `recentSceneProgress` 与 `recentDialogue`：只在旧数据没有可用活动场景时作为兼容回退；正常写作不与 `activeScene` 叠加。
5. `storyHook`：活动场景之前的稳定历史；有可用活动场景时不与当前生活状态竞争。
6. `recentLifeFacts`：活动场景之外、较早时间层的生活锚点。
7. `currentParticipant`（含当前关系态势卡）、`continuityFacts`、Canon、Overlay、剧情余波、到期意图和受限外部上下文。

原始剧本和实际投递消息仍完整保存在数据库。实时请求默认保留最近 20 段剧本及其间完整的已确认收发顺序；任何待处理事件都会固定保留。更早历史由场景外事实、Hook 和长期事实继续承接。
...
插件不会在活动场景中静默删除或合并已发生的剧本文字、用户消息或已投递消息；重复校正只针对当前候选输出发起一次重写，不修改数据库历史。
...
| `interactionLedgerLimit` / `interactionLedgerCharacterBudget` | 近期逻辑回合缓存的数量和预算，默认 18 / 3600；用于重复校正、诊断和缺少活动场景时的兼容回退。 |
