...
> 当前源码版本：0.1.2-beta3-recreate。请先在测试环境中验证服务商、权限、网页浏览和数据清理流程，并妥善保管 API Key 与私聊数据。
...
- 每个关系分支可保存一张短期“关系态势卡”：记录角色对用户当前表现、双方气氛、自己所处环境及下一步交流方式的可修正理解。它会跨越普通闲聊继续生效，但不会改写 Canon 或长期 Overlay。
...
| 剧本引子 / Story Hook | 活动场景之前的生活落点、人物关系、进行中事项和近期事实 | 空闲期低频更新，连接更早历史 |
| 活动场景写作源 | 当前场景最近约 20 段剧本原文与仍待处理的真实事件 | 保留文学细节和动作衔接，同时避免旧回复原文成为下一轮模板 |
| 上一场景尾部 | 最近结束场景的末尾原文 | 为睡醒、换地点、活动结束等转场保留衔接 |
| 近期逻辑回合 | 最近若干次写作的状态、关注面、已完成动作、具体锚点、外部事件影响、用户含义与实际回应含义 | 与活动场景同时提供因果轨迹；最近对话前沿另保留少量真实收发消息，明确哪些话已经说过 |
| 近期生活事实桥 | 活动场景之外、最近 48 小时的生活事实 | 补充昨晚和前一天的时间锚点，并排除已经在活动场景中的内容 |
| 参与者可追溯事实 | 实际收到的消息、白名单人物资料、初始关系与长期事实 | 用户做过什么、说过什么只沿这条来源化通道进入后续事实，不由剧本文字自行确立 |
| 场景状态 | 当前地点、活动、具体动作、已完成/暂停动作、身体、情绪、注意力和未完事项 | 防止每个短回合重新开始同一个动作 |
...
5. 组装单次主模型请求：时间段、当前参与者关系、最近剧本原文、近期状态轨迹、对话前沿、当前真实事件、剧本引子、场景外近期事实、长期事实、其他账号状态、演化覆写和到期意图。最近十个相关回合的真实收发消息作为“已完成对话前沿”提供，较早角色原话不再重复输入。
6. 主模型连续写作：密集对话只追加这几十秒或几分钟真正发生的变化；较长间隔和自动推进再写完整生活段落，随后决定当前时刻角色是否已经看见和回复。
...
同一用户在旧请求刚开始后继续发言时，插件会尝试直接取消该次网络请求，再把新旧消息合并重写；不再只是等待旧响应返回后丢弃。图片读取会并行执行，实时长期事实查询与可选 Embedding 查询也会同时开始。
...
~~~json
{
  ...
  "sceneState": {
    ...
    "label": "晚饭后的厨房收尾",
    "location": "家中厨房",
    "activity": "洗碗并处理刚收到的消息",
    "participants": ["林知遥"],
    "openMatters": ["稍后向用户说明结果"]
  },
  "eventResults": [
    {
      "eventId": "entry:123",
      "status": "responded",
      "effect": "她注意到问候，并决定忙完后回复。"
    }
  ],
  "sceneTrace": {
...
`script` 保存剧本正文，`sceneState` 决定当前场景是否延续，`eventResults` 记录真实事件是否仍待处理，`interaction` 保存机器可执行的互动结果。只有真实用户事件 id 能进入事件状态；只有适配器确认成功的角色消息会成为下一轮既成事实。
...
主模型读取的是“近期剧本原文 + 事实轨迹 + 分层历史”的连续性包：

1. `activeScene`：当前场景最近若干段剧本原文，以及仍未看见或未处理的真实用户事件。已送达角色消息原文不在这里重放。
2. `previousSceneTail`：上一场景末尾，用于延续转场后的动作、情绪和未完成因果。
3. `currentEvent.messages`：本轮新收到的唯一实时事件。相同条目会从 `activeScene` 中暂时排除，避免重复。
4. `recentSceneProgress`：最近逻辑回合的状态、生活关注面、完成动作、具体锚点、真实外部事件造成的影响、未完事项和交流含义，提供稳定的发展轨迹。
5. `recentDialogue`：最近相关回合的用户原话、已实际投递的主角消息、已完成交流动作和待解决问题，用于处理“刚才”“那个”“我说过”等指代，并明确下一句话应从哪里继续。该部分有独立字符预算，并去除与状态轨迹重复的字段，避免真实对话前沿把整份请求重复一遍。
6. `storyHook`：活动场景之前的稳定历史，保持到对话结束小修或长时间空闲刷新。
7. `recentLifeFacts`：不在近期轨迹中的较早时间锚点；已覆盖条目会去重。
8. `currentParticipant`（含当前关系态势卡）、`continuityFacts`、Canon、Overlay、剧情余波、到期意图和受限外部上下文。

原始剧本和实际投递消息仍完整保存在数据库。实时请求默认读取最近 20 段剧本，并保留最近十个相关回合的真实对话作为已经完成的事实；更早角色原话由交流含义替代。任何待处理事件都会固定保留，较早历史由逻辑回合、事实桥、Hook 和长期事实继续承接。

每次写作还会按真实经过时间选择策略：约 2 分钟内只追加同一物理时刻的新动作和对话增量；2～30 分钟推进一个紧凑生活片段；30 分钟～2 小时组织一小段按时序发展的生活；超过 2 小时选择若干关键时刻推进。插件同时发送机器可读的 `writingScope`，记录实际间隔、连续性范围和本轮时间线形状，并在诊断日志中输出实际命中的尺度。最近 12 个逻辑回合的生活关注面会被本地汇总为 `focusBalance`；每轮只保留真正发生变化的一到两个关注面，降低通用标签长期占满上下文的概率。

最近对话中的已投递消息、已完成提问/回答和待解决问题会作为既成状态进入下一轮。若主模型仍直接复用近期整组回复，或把上一段剧本近乎原样重写，插件会使用同一次主叙事协议做一次针对性纠正。该纠正只在检测到明显停滞时触发。服务商失败产生的 `narrative-retry` 会绑定原始用户事件并恢复原回合，不会作为新的到期剧情重新解释一次。

关系态势卡由同一次主叙事请求更新，不增加模型调用。它区分用户实际表达、角色推断和共同事件，用户相关判断必须引用当前真实事件 id。卡片中的 `alreadyExpressed` 保存已经说过的关心、建议或立场，使后续回复继续发展，而不是每轮重新问同一个问题。对话后续阶段把历史消息标记为已完成上下文；只有模型给出新的 `sceneTrace.exchange.newMove` 时，才允许再次发送后续消息。

如果主模型曾连续写出两段几乎相同的剧本，实时上下文只保留其中较新的文学版本；两个回合各自的用户事件、动作结果和交流含义仍保留在事实轨迹中。该处理不删除数据库原文。

这条分层还区分“剧本中的主角感受”与“参与者已发生的行为”。例如，主角可以在正文里猜测某人是否忙碌、期待对方回复；这种主观内容不会自动成为系统认定的用户事实。只有实际入站消息、明确的参与者配置或已保存的长期事实可以作为后续参与者事实的来源。

剧本引子使用两级更新。对话期间由活动场景承载完整细节；最后一次约第 10/20 分钟短期推进输出小型 `storyHookPatch`，连续空闲达到 `memory.storyHookFullRefreshIdleMinutes`（默认 240 分钟）后才完整更新。整个过程复用原有主叙事响应，不增加独立模型调用。

### 6.1 后台压缩器

达到下面任一条件时，活动场景可以进行增量归档整理：

- 未压缩剧本条目达到 memory.sceneEntryThreshold。
- 未压缩内容字符数达到 memory.sceneCharacterThreshold。
- 管理员执行 interlude.compact。

压缩器是独立的低成本模型调用，建议使用更便宜、低温度的模型。它增量合并摘要，但不会自行决定切换场景；只有主叙事已经在正文中写出真实转场并返回 `close-and-open` 时，旧场景才会关闭。分段消息未投递完毕时转场会暂缓。压缩器输出：
...
./plugins/hds-interlude:default:
  model:
    ...
    failover:
      ...
      cooldownMinutes: 5
    mainPrompt: >-
      持续创作一部以主角为中心的现实主义生活剧本，让日程、具体行动、
      身体节奏、兴趣、配角关系、现实压力和外部变化共同推动时间。
    formatPrompt: ''
    fixedPrompt: ''
    stylePrompt: >-
      采用贴近主角的第三人称现实主义叙事，以具体行动、功能性的生活细节、
      配角自主行动和渐进的情绪后果形成生活感。
    compaction:
      enabled: true
      modelId: compact
      ...
      responseFormat: json-object
      mainPrompt: >-
        把已完成剧情整理为连续的事实脉络，保留行动结果、外部事件影响、
        具体生活锚点、承诺、未解决事项和渐进变化。
      fixedPrompt: ''
      stylePrompt: 按时间顺序陈述事实，表达简洁具体，保留仍影响后续的细节。
    embedding:
      enabled: true
      modelId: embedding
  ...
  runtime:
    ...
    # 扫描活动场景后，取最近 20 段剧本与所有待处理事件
    activeSceneEntryLimit: 400
    activeSceneNarrativeLimit: 20
    activeSceneCharacterBudget: 60000
    previousSceneTailCharacters: 4000
    # 近期逻辑回合与场景外事实扫描预算
    contextEntryLimit: 30
    contextCharacterBudget: 6000
    interactionLedgerLimit: 18
    interactionLedgerCharacterBudget: 3600
    # 近期生活事实桥：保留今天、昨晚和前一天的动作与约定，不回传旧正文
    recentLifeFactsEnabled: true
    recentLifeFactHours: 48
    recentLifeFactLimit: 24
    recentLifeFactCharacterBudget: 3200
...
| `activeSceneEntryLimit` | 活动场景数据库扫描上限，默认 400；它用于找到足够多的剧本和待处理事件，不会把所有运输条目都发给模型。 |
| `activeSceneNarrativeLimit` / `activeSceneCharacterBudget` | 最近剧本段落数和字符预算，默认 20 / 60000；前者通常保持 20 即可。 |
| `previousSceneTailCharacters` | 上一场景末尾的保留字符数，默认 4000，用于衔接真实转场。 |
| `contextEntryLimit` / `memoryLimit` | 兼容扫描的近期记录数/带给主模型的长期事实数，默认 30 / 20。 |
| `contextCharacterBudget` | 兼容回退与可追溯事实预算，默认 6000，不控制活动场景原文。 |
| `interactionLedgerLimit` / `interactionLedgerCharacterBudget` | 近期逻辑回合数量和预算，默认 18 / 3600；与剧本原文同时提供动作、具体锚点、外部事件影响和交流轨迹。最近对话前沿会额外保留少量真实收发消息。 |
| `recentLifeFactsEnabled` / `recentLifeFactHours` | 是否启用最近生活事实桥，以及覆盖多少小时。默认开启 / 48 小时；它用于保留今天、昨晚和前一天的生活事实。 |
| `recentLifeFactLimit` / `recentLifeFactCharacterBudget` | 最多保留多少条较早生活事实及其字符预算。默认 24 / 3200；提高可增强跨日连贯性，也会增加上下文。 |
...
| `staleNarrativeRequestWindowSeconds` | 默认 5 秒。窗口内的新消息会使正在运行的旧主模型请求过期，并在服务商支持取消信号时停止该网络请求；随后以合并后的真实消息重新写作。 |
...
| interlude.context | 查看活动场景统计、待处理事件、上一场景尾部、剧本引子、演化覆写和已检索事实 |
...
## 0.1.2-beta3-recreate：分阶段生活写作提示词

- 主叙事固定提示词改为以正面创作要求为主，强化具体行动、功能性生活细节、身体节奏、配角自主行动、现实压力和外部事件影响。
- 用户消息、对话后续、独立生活推进和到期意图分别使用当前阶段专用要求，不再同时发送其他阶段的无关写作说明。
- 即时、短段、中段和长段写作分别使用当前时间尺度专用要求，密集对话聚焦新增变化，长间隔选择有因果连接的生活时刻。
- 强化回复语义推进与 `sceneTrace.exchange.responseMeaning`，让连续气泡共同完成一个有进展的表达，并为后续回合保存实际传达内容。
- 更新主叙事与压缩模型的 Console 默认提示词；压缩器进一步保留生活锚点、外部事件影响和仍作用于当前场景的细节。

## 0.1.2-beta5-recreate：关系态势与已完成交流修复

- 新增每个参与者独立的“关系态势卡”。它保存一段关系中仍在起作用的用户表现、角色处境、交流方向、已表达内容和到期时间，用于让关心、紧张、冲突或承诺在普通闲聊后仍能自然延续。
- 关系态势由同一次主叙事结构化输出更新，不增加额外模型请求；涉及用户的判断必须引用本轮真实事件 id，避免将剧本文字误当成用户事实。
- 将已发送的关心、建议和立场写入 `alreadyExpressed`，后续回合优先推进新的交流动作，而不是重新追问或重复同一层意思。
- 重构对话后续的事件语义：历史收发消息明确作为已完成上下文；只有模型给出新的交流动作时，短期跟进才允许再次投递可见消息。
- 补充关系态势的 Console 配置、上下文查看、测试步骤和运行边界说明。

## 0.1.2-beta2-refactor：活动场景连续性重构

- 主叙事使用最近剧本原文、近期逻辑回合和对话前沿共同维持短期连续性；上一场景尾部负责转场衔接。
- 新增 `sceneState` 与 `eventResults` 协议，由主模型按生活阶段判断场景边界，并逐条记录真实消息的处理状态。
- 松散短句继续属于当前场景；未看见的真实事件会保留到后续回合，不再依赖摘要猜测是否处理。
- 最近已投递角色消息以“已完成对话前沿”输入，帮助模型识别已经问过或强调过的内容；更早措辞只保留事实含义，当前事件保持唯一来源。
- 密集对话采用增量写作，场景状态记录当前动作、已完成/暂停动作、身体、情绪和注意力。
- 连续复制上一回合回复时执行一次纠正重写；分段消息按回合序号串行投递。
- 角色消息只有适配器确认投递后才成为连续性事实，避免未发送草稿或失败分段污染后续剧本。
- 场景归档使用主叙事写下的真实转场作为边界；分段消息未发完或压缩尚未覆盖边界时不会提前关场。
- 活动场景条目、字符预算和上一场景尾部长度可在 Console 调整；默认优先保证连贯性。
- 补充已传达语义记录，帮助主模型在下一轮继续推进含义，减少自动推进和连续对话中的同义复述。
