...
| 近期生活事实桥 | 最近 36 小时的时间、地点、行动、人物互动、约定和未完事项 | 让模型能衔接昨晚和当天较早的生活，不需要重新输入旧剧本文字 |
...
5. 组装单次主模型请求：时间段、当前参与者关系、剧本引子、近期逻辑回合卡、近期生活事实桥、可追溯的参与者事实、其他账号状态、基础设定、演化覆写和到期意图；回合卡会合并分段消息，并只记录实际投递成功的主角消息。主模型不再读取成批历史剧本正文。长期事实会按当前消息、未解决线索和到期计划进行召回与重排。
...
2. `recentLogicalTurns`：近期完整回合的事实卡。每张卡保留主角刚完成的行动、地点、配角、小物件、约定、未完事项，以及私聊中“用户表达了什么 / 主角已经传达了什么”的语义结果。用户的原始消息仍作为事实依据保留；已投递的主角原句只保留在数据库审计中，不会反复作为主模型的措辞示例。
3. `recentLifeFacts`：最近一天左右的生活事实桥。它从较早回合中保留发生时间、主角行动、地点、人物互动、承诺和未完事项，用于理解“昨晚”“今天早些时候”等连续时间引用；其中不含旧剧本文字或聊天原句。
4. `currentParticipant`：当前账号的资料、初始关系、关系状态与开放话题；它是这类基线信息在实时请求中的唯一副本。
5. `currentEvent.messages`：当前真实入站事件及其来源 id，用于保存有依据的参与者事实，不重复传送资料、关系或长期事实。
6. `continuityFacts`：去重后的长期事实与记忆集合，经重要度、可信度、时间、未解决状态与可选 Embedding 重排。
7. Canon、Overlay、剧情余波、到期意图和网页观察。
8. 本回合唯一的 `currentEvent`。

原始剧本仍完整保存在数据库，并继续供时间线、审计和后台压缩使用；它不再批量进入实时主叙事请求。最近对话则以“原始用户事件 + 已处理的语义结果 + 当前生活状态 + 最近生活事实桥”的形式进入写作上下文：既能处理省略、追问、昨晚发生的事和刚完成的动作，也能避免模型持续模仿自己上一轮的短回复、段落结构和固定动作。
...
./plugins/hds-interlude:default:
  ...
  runtime:
    ...
    # 最近一天的生活事实桥：保留昨晚/当天早些时候的动作与约定，不回传旧正文
    recentLifeFactsEnabled: true
    recentLifeFactHours: 36
    recentLifeFactLimit: 20
    recentLifeFactCharacterBudget: 2400
...
| `recentLifeFactsEnabled` / `recentLifeFactHours` | 是否启用最近生活事实桥，以及覆盖多少小时。默认开启 / 36 小时；它用于保留昨晚和当天早些时候的生活事实。 |
| `recentLifeFactLimit` / `recentLifeFactCharacterBudget` | 最多保留多少条较早生活事实及其字符预算。默认 20 / 2400；提高可增强日内连贯性，也会增加上下文。 |
...
| interlude.context | 查看剧本引子、近期逻辑回合卡、近期生活事实桥、活动场景、演化覆写和已检索事实 |
