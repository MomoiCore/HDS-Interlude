...
const StoryDefaults: Schema<StoryDefaults> = Schema.object({
  ...
  characterProfile: Schema.string().role('textarea').default('18岁的女孩，刚刚高考结束，正准备开始大学生活。平时喜欢熬夜，有点内向，容易胆怯，生活比较丰富，喜欢尝试一些能力范围内没试过的东西，对想干的事情非常有行动力，在线上聊天话很少且发言简洁、有点喜欢吐槽，但对待事情十分认真，心态很平和').description('主角的背景、性格、日程和说话方式；作为故事起点，不是永久锁定的人设。若这里发生大幅修改，请保存后执行 interlude.overlay.clear character，随后按提示输入 y 确认；小幅补充、措辞调整或细节修正无需其它操作。'),
  ...
  world: Schema.string().role('textarea').default('现实社会，主角平常生活在中国').description('故事时代、地点和现实规则；作为剧本的初始世界状态。若大幅改写世界前提，请执行 interlude.overlay.clear world，随后按提示输入 y 确认；小幅补充无需处理。'),
  supportingCast: Schema.string().role('textarea').default('主角的父母，对主角比较严格，工作早出晚归；主角的一个亲姐姐，大主角3岁；主角的一位好友，名叫希绘（Nozomi），比较外向，比主角小一点，两人非常要好。').description('配角及其与主角的关系；无配角可留空。'),
  location: Schema.string().default('').description('主角的主要活动地点。'),
  style: Schema.string().role('textarea').default('当代现实主义生活剧。主角拥有丰富、具体且持续变化的个人生活；配角也有各自的节奏、立场与情绪。关系在日常互动和小事件中缓慢发展，生活常常留下未完成但真实感的余波。').description('该主剧本的文风；优先级高于全局 stylePrompt。'),
